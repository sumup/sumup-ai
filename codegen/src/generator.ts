...
export type CodegenOptions = {
  ...
  registrationExclusions?: Record<string, string>;
};
...
export async function generate(
  ...
  await writeRegistryFile(
    options.outputDir,
    entries,
    options.registrationExclusions ?? {},
  );
}
...
async function writeRegistryFile(
  outputDir: string,
  entries: [string, OperationDetails[]][],
  registrationExclusions: Record<string, string>,
) {
  const parts = [
  ...
  const registeredOperations: OperationDetails[] = [];
  const excludedOperations: OperationDetails[] = [];

  for (const [tag, operations] of entries) {
    const registered = operations.filter(
      (operation) => !(operation.operationId in registrationExclusions),
    );
    ...
    registeredOperations.push(...registered);

    if (registered.length > 0) {
      parts.push(
        `import { ${registered.map((operation) => operation.toolConst).join(", ")} } from "./${kebabCase(tag)}";`,
      );
    }
  }

  registeredOperations.sort((a, b) => a.toolConst.localeCompare(b.toolConst));
  excludedOperations.sort((a, b) => a.toolConst.localeCompare(b.toolConst));
  ...
  parts.push(
    ...
    ...excludedOperations.map(
      (operation) =>
        `  ${JSON.stringify(operation.toolName)}: ${JSON.stringify(registrationExclusions[operation.operationId])},`,
    ),
    "} as const;",
    "",
    "export const registerTools = (reg: (tool: Tool) => void) => {",
    ...registeredOperations.map(
      (operation) => `  reg(${operation.toolConst});`,
    ),
    "};",
  );

  const content = await format(parts.join("\n"), { parser: "typescript" });
  writeFileSync(join(outputDir, "registry.ts"), `${content}\n`);
}
